...
// Basis: RAW = data/citations.tsv (one file, N≈10140); GOLD (--gold) = the five
// frozen gold sets data/derived/gold-<emonym>.tsv (N≈9984), each with its own header.
const GOLD = process.argv.includes('--gold');
const SRC_FILES = GOLD
  ? EM.map(e => `${REPO}/data/derived/gold-${e}.tsv`)
  : [`${REPO}/data/citations.tsv`];
...
for (const file of SRC_FILES) {
  const tsv = fs.readFileSync(file, 'utf8').split(/\r?\n/).filter(Boolean);
...
// ---- sanity ----
const AUDIT = GOLD ? {   // gold Sᵢ from data/derived/distribution-frequency-mass.md
  'Res Liquidae': { miedo: 45, tristeza: 155, amor: 562, 'alegría': 506, ira: 45 },
  'Res Filiformes': { miedo: 172, tristeza: 38, amor: 207, 'alegría': 1201, ira: 1565 },
  'Res Continens': { miedo: 176, tristeza: 122, amor: 132, 'alegría': 71, ira: 30 },
  'Res Parvae': { miedo: 546, tristeza: 50, amor: 176, 'alegría': 3793, ira: 11 } } : {
  'Res Liquidae': { miedo: 45, tristeza: 159, amor: 579, 'alegría': 519, ira: 45 },
...
const O = [];
const P = s => O.push(s);
P('# Cryptoclass indices for the Spanish emonyms (ПоКА · ПА · ИРа · СИ)' + (GOLD ? ' — GOLD basis' : ''));
P('');
P('Generated by `pipeline/compute_indices.js' + (GOLD ? ' --gold`' : '`') + ' from '
  + (GOLD ? 'the five frozen gold sets `data/derived/gold-<emonym>.tsv` (GOLD basis, N≈9984)'
          : '`data/citations.tsv` (RAW basis, N≈10140)') + ' and the');
P('vetted classifier inventory. Base-lemma counting (option 1). `nivel de` and');
...
for (const e of EM) { P(`**${e}**`); P('');
  ...
  P(''); }
const OUT_MD = '/data/derived/cryptoclass-indices' + (GOLD ? '-gold' : '') + '.md';
fs.writeFileSync(REPO + OUT_MD, O.join('\n'));
...
console.log('\nWrote' + (GOLD ? ' [GOLD]' : '') + ' data/derived/cryptoclass-indices' + (GOLD ? '-gold' : '') + '.md');
